...
### `@orpc/nest` subpath imports

In v2, `@orpc/nest` exports dedicated subpath entrypoints for OpenAPI and RPC protocols. See [NestJS Integration](/docs/integrations/nest).

<CodeGroup>

```ts v2
import { Implement, ORPCModule } from '@orpc/nest/openapi' // or '@orpc/nest/rpc'
```
...
:::info
To keep the exact same behavior as v1 (OpenAPI routing), import from `@orpc/nest/openapi`. Use `@orpc/nest/rpc` if you want native RPC protocol handling over HTTP POST.
:::
