...
:::warning
Only register this on an `RPCHandler` if every client is trusted. Deserializing a `RegExp`
compiles an attacker-controlled pattern, and every later use of it runs an attacker-controlled
matcher — `^(a+)+$` is eight characters and takes over a second to test a 28-character string.
This runs before your middleware, so authentication cannot gate it. On an `RPCLink` it only
exposes you to your own server, which is usually fine.
:::
