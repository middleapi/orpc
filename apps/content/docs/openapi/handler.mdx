...
Two things to keep in mind when you pass a function:

- If it throws, `handle` rejects instead of returning a response. It runs before routing, so no
  procedure is matched and neither the [routing interceptors](#routing-interceptors) nor the
  codec's error encoding ever see the error, which means nothing turns it into a response.
  [Adapter interceptors](#adapter-interceptors) do observe the rejection, since they wrap the
  handler. Handle it yourself, or keep the work that can fail inside a
  [middleware](/docs/middleware) so it is encoded like any other error.
- [Adapter interceptors](#adapter-interceptors) receive `context` exactly as you passed it, so
  spreading it (`{ ...options.context }`) silently yields `{}` when it is a function. Compose it
  instead:
