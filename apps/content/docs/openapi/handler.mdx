...
- If it throws, `handle` rejects instead of returning a response. It runs before routing, so no
  procedure is matched and neither the [routing interceptors](#routing-interceptors), the plugins,
  nor the codec's error encoding ever see the error. Handle it yourself, or keep the work that can
  fail inside a [middleware](/docs/middleware) so it is encoded like any other error.
- Adapter-level interceptors receive `context` exactly as you passed it, so spreading it
  (`{ ...options.context }`) silently yields `{}` when it is a function. Compose it instead:
