...
:::info
The response is exposed only through `cause`, never through the error's `data`. `data` is serialized by `ORPCError.toJSON()`, so a link used inside a server would otherwise forward the upstream response headers and body on to its own callers.
:::
