...
The promise it receives rejects when a refresh fails, so `cache/waitUntil` is also where those failures are reported. Without it the refresh still runs, but its failures are ignored and the next stale hit retries.
