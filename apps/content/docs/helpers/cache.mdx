...
| `KVCacheStore`      | [Cloudflare Workers KV](https://developers.cloudflare.com/kv/)                                                                                                       |
| `WorkersCacheStore` | [Cloudflare Workers Caching](https://developers.cloudflare.com/workers/cache/), purge only                                                                          |
...
```ts cloudflare-kv
import { KVCacheStore } from '@orpc/cloudflare'
...
import { WorkersCacheStore } from '@orpc/cloudflare'
...
export default {
  async fetch(request, env, ctx) {
    ...
    const store = new WorkersCacheStore(ctx.cache)
  },
}
```
