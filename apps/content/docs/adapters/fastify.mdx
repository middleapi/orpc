...
Fastify's default 1 MiB [`bodyLimit`](https://fastify.dev/docs/latest/Reference/Server/#bodylimit)
is enforced by the parsers you just removed, so it no longer applies — oRPC reads the body
itself, before any middleware runs. Restore a limit with the
[Request Limit Plugin](/docs/plugins/request-limit):
