...
```ts
import { createHTTPUtils } from '@orpc/msw'
...
The `origin` and `prefix` options must match where your [RPC Link](/docs/rpc/link) sends requests. `origin` supports MSW wildcards, such as `*` to match any origin.

:::info
When you mock an implemented router, only the mock implementations run. The real handlers and middleware are never invoked. [Lazy routers](/docs/router#lazy-router) are not supported directly, [unlazy the router](/docs/contract/router#router-to-contract) first.
:::
...
If your client uses the [OpenAPI Link](/docs/openapi/link), create an [OpenAPI Handler](/docs/openapi/handler) instead. Requests are then matched against each procedure's [route](/docs/openapi/routing), including custom methods, prefixes, and path parameters such as `/planets/{id}` or greedy `/files/{+path}` segments.
...
The `context` option controls the [context](/docs/context) passed to the created handler for each request, and mock handlers receive it as `context`, enabling context-driven behaviors such as the [Response Headers Plugin](/docs/plugins/response-headers):
