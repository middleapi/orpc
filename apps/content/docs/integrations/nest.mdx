...
description: "Learn how to implement oRPC contracts in NestJS with the @orpc/nest package while keeping type safety and OpenAPI or RPC protocol compatibility."
sidebar:
  label: "NestJS"
---
...
Before implementation, define your [contract](/docs/contract/procedure) as usual, including [OpenAPI Routing](/docs/openapi/routing) or [RPC Protocol](/docs/rpc/protocol). There is no special requirement, except that each OpenAPI contract must define an `openapi.path` meta.
...
To implement your contract in NestJS, use the `@Implement` decorator and the `implement` function. `@orpc/nest` exports dedicated subpath entrypoints for [OpenAPI Handler](/docs/openapi/handler) (`@orpc/nest/openapi`) and [RPC Handler](/docs/rpc/handler) (`@orpc/nest/rpc`).

The `@Implement` decorator works very similarly to NestJS built-in HTTP method decorators (e.g., `@Get`, `@Post`) and can be used to implement either a single procedure contract or a [router contract](/docs/contract/router), or combine with other NestJS decorators.

```ts
import { Implement } from '@orpc/nest/openapi' // or '@orpc/nest/rpc'
...
Configure `@orpc/nest` by importing `ORPCModule` from `@orpc/nest/openapi` or `@orpc/nest/rpc` into your NestJS module. It supports the same options as the [OpenAPI Handler](/docs/openapi/handler) or [RPC Handler](/docs/rpc/handler), except for options that are unrelated to NestJS and options that are specific to NestJS.

<CodeGroup>

```ts Static
import { onError } from '@orpc/server'
import { ORPCModule } from '@orpc/nest/openapi' // or '@orpc/nest/rpc'
...
```ts
import { NestStandardLazyRequest } from '@orpc/nest/openapi' // or '@orpc/nest/rpc'
...
After implementing your contract in NestJS, you can use the same contract to create a typesafe client. See [OpenAPI Link](/docs/openapi/link) or [RPC Link](/docs/rpc/link) for more details.
