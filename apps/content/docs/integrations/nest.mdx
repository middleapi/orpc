...
:::info
Contracts that use the HTTP [`QUERY`](/docs/openapi/routing) method are supported when NestJS provides `@QueryMethod` (NestJS v11.2+). On older NestJS versions, `@Implement` throws a `TypeError` for `QUERY` contracts — use `GET` instead.
:::
