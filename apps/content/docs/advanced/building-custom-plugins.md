...
Check out the [built-in handler plugins](https://github.com/middleapi/orpc/tree/1.x/packages/server/src/plugins) to see real-world examples of how different plugins solve common server-side challenges.
...
Browse the [built-in link plugins](https://github.com/middleapi/orpc/tree/1.x/packages/client/src/plugins) for inspiration on handling common client-side scenarios.
...
The [Strict Get Method Plugin](/docs/plugins/strict-get-method) ([Source Code](https://github.com/middleapi/orpc/blob/1.x/packages/server/src/plugins/strict-get-method.ts)) demonstrates this pattern. It uses `rootInterceptors` to collect HTTP methods and combines this data with procedure information in `clientInterceptors` to determine whether the method is allowed.
